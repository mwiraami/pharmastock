import { productTotals, quantityInTablets } from "./business-rules.js";
...
let db, syncTimer, state={pharmacy:null,currentUser:null,products:[],batches:[],cart:[],page:"dashboard",openBatchForm:false,editProductId:null,api:{available:false,accessToken:null,refreshToken:null},installPrompt:null};
...
function updateConnectionStatus(){document.querySelectorAll(".offline").forEach(element=>{element.textContent=navigator.onLine?"● En ligne":"● Hors connexion"})}
window.addEventListener("online",async()=>{await detectApi();updateConnectionStatus()});
window.addEventListener("offline",()=>{state.api.available=false;updateConnectionStatus()});

function scheduleSync(){clearTimeout(syncTimer);syncTimer=setTimeout(()=>{if(state.currentUser)render()},120)}
function notifyChange(store){let change={store,time:Date.now()};syncChannel?.postMessage(change);localStorage.setItem(SYNC_KEY,JSON.stringify(change))}
syncChannel?.addEventListener("message",scheduleSync);
window.addEventListener("storage",event=>{if(event.key===SYNC_KEY)scheduleSync()});
...
async function products(){if(state.api.available&&state.api.accessToken){try{let remoteProducts=await apiRequest("/api/products");if(remoteProducts.length&&state.batches.length)state.products=remoteProducts}catch(error){note(error.message)}}let totals=productTotals(state.products,state.batches);shell(`<div class="panel-head"><h2>Produits</h2><button id="new">Enregistrer un produit</button></div><div class="grid"><div class="card"><p>Stock total</p><strong>${totals.stock} comprimés</strong></div><div class="card"><p>Valeur achat</p><strong>${fmt(totals.buyValue)}</strong></div><div class="card"><p>Valeur vente</p><strong>${fmt(totals.saleValue)}</strong></div><div class="card"><p>Marge potentielle</p><strong>${fmt(totals.saleValue-totals.buyValue)}</strong></div></div><section class="panel"><label>Rechercher<input id="search" placeholder="Nom, code ou code-barres"></label></section><section class="panel" id="form" hidden><form id="f" class="row">${fields([["code","Code produit","text",1],["barcode","Code-barres","text"],["name","Nom commercial","text",1],["genericName","Nom générique","text"],["category","Catégorie","text"],["form","Forme pharmaceutique","text"],["dosage","Dosage","text"],["unit","Unité","text"],["packSize","Comprimés par plaquette","number",1,"1"],["manufacturer","Fabricant","text"],["supplier","Fournisseur","text"],["buyPrice","Prix achat / comprimé","number",1],["salePrice","Prix vente / comprimé","number",1],["buyPackPrice","Prix achat / plaquette","number",1],["salePackPrice","Prix vente / plaquette","number",1],["minStock","Stock minimum","number",1,"0"],["maxStock","Stock maximum","number"],["location","Emplacement","text"],["prescription","Ordonnance obligatoire (oui/non)","text"],["status","Statut ACTIVE/INACTIVE","text",1,"ACTIVE"]])}<button id="save-product">Enregistrer</button><button type="button" id="cancel-product" class="secondary">Annuler</button></form></section><section class="panel" id="product-profile" hidden></section><div id="product-table">${productTable(state.products)}</div>`);
$("#new").onclick=()=>$("#form").hidden=false;
$("#cancel-product").onclick=()=>{$("#form").hidden=true;state.editProductId=null};
...
let x=Object.fromEntries(new FormData(e.target));
x={id:state.editProductId||id(),...x,status:String(x.status||"ACTIVE").toUpperCase(),buyPrice:+x.buyPrice,salePrice:+x.salePrice,buyPackPrice:+x.buyPackPrice,salePackPrice:+x.salePackPrice,minStock:+x.minStock,maxStock:+x.maxStock,packSize:Math.max(1,Math.trunc(+x.packSize||1)),updatedAt:new Date().toISOString()};
...
let index=state.products.findIndex(product=>product.id===x.id);
if(index===-1)state.products.push(x);else state.products[index]=x;
await audit(state.editProductId?"Modification produit":"Création produit","Produits",{name:x.name});
state.editProductId=null;
note("Produit enregistré");
products()};
bindArchives()}
function productTable(list){return table(["Produit","Code","Stock","Achat comprimé","Vente comprimé","Achat plaquette","Vente plaquette","Statut","Action"],list.map(p=>[esc(p.name),esc(p.barcode||p.code),`${available(p.id)} comprimés`,fmt(p.buyPrice),fmt(p.salePrice),fmt(p.buyPackPrice||p.buyPrice*(p.packSize||1)),fmt(p.salePackPrice||p.salePrice*(p.packSize||1)),esc(p.status),`<button class="secondary profile" data-id="${p.id}">Profil</button> <button class="secondary edit-product" data-id="${p.id}">Modifier</button> <button class="danger delete-product" data-id="${p.id}">Supprimer</button>`]))}
function bindArchives(){document.querySelectorAll(".edit-product").forEach(button=>button.onclick=()=>{let product=state.products.find(item=>item.id===button.dataset.id);state.editProductId=product.id;let form=$("#f");Object.entries(product).forEach(([key,value])=>{let field=form.elements[key];if(field)field.value=value??""});$("#form").hidden=false;window.scrollTo({top:0,behavior:"smooth"})});document.querySelectorAll(".delete-product").forEach(button=>button.onclick=async()=>{let product=state.products.find(item=>item.id===button.dataset.id);if(!product||!confirm(`Supprimer ${product.name} ?`))return;if(state.batches.some(batch=>batch.productId===product.id&&Number(batch.available)>0))return note("Impossible de supprimer un produit avec du stock disponible. Archivez-le plutôt.");await new Promise((resolve,reject)=>{let request=db.transaction("products","readwrite").objectStore("products").delete(product.id);request.onsuccess=resolve;request.onerror=()=>reject(request.error)});state.products=state.products.filter(item=>item.id!==product.id);await audit("Suppression produit","Produits",{name:product.name});note("Produit supprimé");products()});document.querySelectorAll(".profile").forEach(button=>button.onclick=()=>{let product=state.products.find(item=>item.id===button.dataset.id),stock=physicalStock(product.id),packSize=product.packSize||1,profile=$("#product-profile");profile.hidden=false;profile.innerHTML=`<div class="panel-head"><h3>Profil : ${esc(product.name)}</h3><button type="button" id="close-profile" class="secondary">Fermer</button></div><p><strong>Code :</strong> ${esc(product.code||"—")} · <strong>Forme :</strong> ${esc(product.form||"—")} · <strong>Dosage :</strong> ${esc(product.dosage||"—")}</p><p><strong>Stock :</strong> ${stock} comprimés · <strong>Plaquette :</strong> ${packSize} comprimés</p><p><strong>Achat :</strong> ${fmt(product.buyPrice)} / comprimé · ${fmt(product.buyPackPrice||product.buyPrice*packSize)} / plaquette</p><p><strong>Vente :</strong> ${fmt(product.salePrice)} / comprimé · ${fmt(product.salePackPrice||product.salePrice*packSize)} / plaquette</p><p><strong>Valeur achat :</strong> ${fmt(stock*product.buyPrice)} · <strong>Valeur vente :</strong> ${fmt(stock*product.salePrice)}</p>`;$("#close-profile").onclick=()=>profile.hidden=true})}
...
if(i)i.quantity+=q;
else state.cart.push({productId:p.id,name:p.name,form:p.form,dosage:p.dosage,quantity:q,enteredQuantity:+x.quantity,unit:x.unit,price:x.unit==="PLAQUETTE"?+(p.salePackPrice||p.salePrice*(p.packSize||1))/(p.packSize||1):+p.salePrice,discount:+x.discount});
sale()};
...
render()}}
function printTicket(s){let rows=s.items.map(i=>`<tr><td>${esc(i.name)}</td><td>${i.enteredQuantity||i.quantity} ${i.unit==="PLAQUETTE"?"plaquette(s)":"comprimé(s)"}</td><td>${fmt(i.quantity*i.price-i.discount)}</td></tr>`).join(""),qrData=encodeURIComponent(JSON.stringify({pharmacyId:state.pharmacy.id||"pharmacy",name:state.pharmacy.name,phone:state.pharmacy.phone||""})),qr=`<img alt="QR pharmacie" width="120" height="120" src="https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${qrData}">`,w=window.open("","_blank","width=430,height=760");
...
font-size:18px;
margin-top:16px}.actions{display:flex;gap:8px;margin:15px 0}.actions button{padding:8px 10px}</style><h1>${esc(state.pharmacy.name)}</h1><p>${esc(state.pharmacy.address||state.pharmacy.city||"")}<br>Ticket : ${s.ticket}<br>${new Date(s.date).toLocaleString("fr-FR")}<br>Caissier : ${esc(s.user||state.pharmacy.admin)}</p><table><tr><th>Article</th><th>Qté</th><th>Total</th></tr>${rows}</table><p class="total">Total : ${fmt(s.total)}</p><p>Paiement : ${esc(s.method)}<br>Payé : ${fmt(s.paid)}<br>Monnaie : ${fmt(s.change)}</p><p>QR de la pharmacie :</p>${qr}<div class="actions"><button onclick="print()">Enregistrer en PDF / Imprimer</button><button onclick="shareReceipt()">Partager</button></div><p>Merci de votre visite.</p><script>async function shareReceipt(){if(!navigator.share)return alert('Le partage n’est pas disponible sur ce navigateur.');await navigator.share({title:'Ticket ${s.ticket}',text:'Ticket ${s.ticket} - ${esc(state.pharmacy.name)} - ${fmt(s.total)}'}).catch(()=>{});}</script>`);
w.document.close()}
